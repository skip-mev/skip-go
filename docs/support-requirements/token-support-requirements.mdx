...
2. The Skip Go API must also support the remote chains to which you wish users to be able to transfer the asset  
    ...
    **Note:** We are unable to support tokens from a source chain that is not integrated with Skip Go. Skip Go requires direct integration with the source chain to query chain state and balances, submit and monitor transactions, calculate accurate gas fees and handle chain-specific token mechanics.
