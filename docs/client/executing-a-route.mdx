...
```appendCosmosMsgs``` Optional Cosmos messages to append, keyed by chain ID (`Record<chainId, CosmosMsg[]>`). Each `CosmosMsg` is `{ msgTypeUrl, msg }`, where `msg` is the JSON-encoded message body.
