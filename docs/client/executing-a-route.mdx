...
```getSvmSigner``` Function that returns a ```Promise<Adapter>``` (Solana wallet-adapter)
...
```appendCosmosMsgs``` Optional Cosmos messages, keyed by chain ID (`Record<chainId, CosmosMsg[]>`). These are inserted before the existing messages in the first Cosmos transaction matching each chain ID. Each `CosmosMsg` is `{ msgTypeUrl, msg }`, where `msg` is the JSON-encoded message body.
...
```getCosmosPriorityFeeDenom``` function that takes a chainId and returns a ```Promise<string | undefined>``` for the priority fee denom to use for Cosmos transactions.
...
```onApproveAllowance``` Called when the allowance tx is being executed (only in evm txs).
...
await executeRoute({
  ...
  appendCosmosMsgs: {
    "osmosis-1": [{ msgTypeUrl: "/cosmos.bank.v1beta1.MsgSend", msg: cosmosMsgJson }],
  },
