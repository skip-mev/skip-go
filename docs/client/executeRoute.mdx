title: 'executeRoute – Executing a Cross-Chain Route'
description: 'This page documents the executeRoute function, used to execute a token transfer or swap across chains using a precomputed route.'
Executing a cross-chain route
The executeRoute function is the core entry point for executing a precomputed route, including optional support for simulating transactions, injecting custom Cosmos messages, and tracking transaction status.

You must provide a list of user addresses (one per participating chain), along with the route object returned from a prior route computation (e.g. from getRoute or getMsgsV2).
...
userAddresses: One user address per participating chain in the same order as route.requiredChainAddresses.
...
batchSimulate: If true, simulate all transactions in a batch before execution; if false, simulate each transaction individually.

beforeMsg / afterMsg: Optional Cosmos messages to inject at the beginning or end of the route execution.

useUnlimitedApproval: If true, sets EVM token allowances to MAX_UINT256 if required.

bypassApprovalCheck: If true, skips approval checks on EVM chains.
