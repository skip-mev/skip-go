...
## Appending custom Cosmos messages to route execution

The `executeRoute` method accepts an `appendCosmosMsgs` parameter to append custom Cosmos messages to the route's transactions, keyed by chain ID (`Record<chainId, CosmosMsg[]>`). This is useful for executing custom messages that are not part of the route definition.
...
await executeRoute({
  ...
  userAddresses,
  appendCosmosMsgs: {
    'cosmoshub-4': [{ msgTypeUrl: '/cosmos.bank.v1beta1.MsgSend', msg }]
  }
});
```
