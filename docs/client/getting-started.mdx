...
- **`setClientOptions(options)`:** Use this for a comprehensive setup, especially if you plan to use `executeRoute` or other functions requiring on-chain interaction. It configures API access (`apiUrl`, `apiKey`, `cacheDurationMs`) and chain-specific details like RPC/REST endpoints (`endpointOptions`), Amino types (`aminoTypes`), and Protobuf registry types (`registryTypes`).
- **`setApiOptions(options)`:** Use this if you primarily need to configure API interaction (`apiUrl`, `apiKey`) and/or set up affiliate fee structures (`chainIdsToAffiliates`). This option does not configure `endpointOptions`, `aminoTypes`, or `registryTypes`.
...
setApiOptions({
  apiUrl: "YOUR_API_URL", // Optional: defaults to Skip API
  apiKey: "YOUR_API_KEY",   // Optional: required for certain features
  // chainIdsToAffiliates: { /* ... */ } // Optional: for affiliate fees
});
...
- `chainIdsToAffiliates?: Record<string, ChainAffiliates>`: Configure affiliate fee details (used by `setApiOptions`).
...
await executeRoute({
  ...
  onTransactionCompleted: async (txInfo) => {
    console.log(
      `Route completed on chain ${txInfo.chainId} with tx hash: ${txInfo.txHash} & status: ${txInfo.status?.state}`
    );
  },
...
- `messagesDirect`: Sign messages directly
...
Remember, if you use `executeRoute` (Step 7), it automatically handles the transaction lifecycle, including waiting for completion. The manual polling method described below using `transactionStatus` is primarily for scenarios where you are not using `executeRoute` for full execution (e.g., if you use `submitTransaction` directly) or if you need more granular control over the tracking process.
...
async function handleManualTx(signedTx: any, chainId: string) {
  ...
  // Poll for status using transactionStatus
  const checkStatus = async () => {
    try {
      const statusResult = await transactionStatus({ txHash, chainId });
      console.log("Current status:", statusResult.state, statusResult);
      ...
      if (statusResult.state === "STATE_COMPLETED_SUCCESS") {
        console.log("Final status (Completed Successfully):", statusResult);
      } else if (statusResult.state === "STATE_COMPLETED_ERROR" || statusResult.state === "STATE_ABANDONED") {
        console.error("Final status (Failed or Abandoned):", statusResult.error || "Abandoned");
      } else {
        console.log(`Transaction for ${txHash} on ${chainId} is still processing with state: ${statusResult.state}. Polling again in 5s.`);
        setTimeout(checkStatus, 5000);
      }
    } catch (error) {
      console.error(`Error polling for status of ${txHash} on ${chainId}:`, error);
      // Decide if you want to retry polling on error or stop.
    }
  };
