...
// Balances are requested per chain, keyed by chain ID.
// Provide `denoms` to fetch specific assets, or an empty array to fetch all.
// returns balances grouped by chain ID (see BalanceResponse)
