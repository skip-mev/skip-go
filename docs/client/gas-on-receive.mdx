...
Gas on Receive prevents users from getting "stuck" with assets they can't use by automatically providing a small amount of native gas tokens on the destination chain. The client library provides two approaches:

1. **Automatic** - Use `getRouteWithGasOnReceive` to automatically split routes
2. **Manual** - Full control over gas balance detection and route creation
...
async function swapWithGasOnReceive() {
  ...
  // Step 2: Automatically split into main and gas routes
  const { mainRoute, gasRoute } = await getRouteWithGasOnReceive({
    ...
    routeRequest: {
      ...
      destAssetDenom: "0x82aF49447D8a07e3bd95BD0d56f35241523fBab1",
      smartRelay: true
    }
  });

  // Step 3: Get user addresses for required chains
  const userAddresses = [
  ...
  if (gasRoute) {
    ...
    await executeMultipleRoutes({
      ...
      userAddresses: {
        ...
        gasRoute: userAddresses // Same addresses for both routes
      },
      slippageTolerancePercent: {
        mainRoute: "1",
        gasRoute: "10" // Higher tolerance for gas route
      },
      ...
      onRouteStatusUpdated: (status) => {
        ...
        const gasRouteFailed = status.relatedRoutes?.find(
          r => r.routeKey === "gasRoute" && r.status === "failed"
        );
...
1. Checks if the destination chain is supported (excludes Ethereum mainnet and Solana)
2. Verifies the destination asset isn't already a fee asset
3. Calculates appropriate gas amounts ($0.10 for Cosmos, $2.00 for EVM L2s)
4. Creates a gas route to obtain native tokens
5. Adjusts the main route amount accordingly
6. Returns both routes or just the original if gas isn't needed

## Manual Implementation Guide

For full control over the Gas on Receive logic:
