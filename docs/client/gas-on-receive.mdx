  ...
  if (gasRoute && gasRouteAddresses) {
    ...
    await executeMultipleRoutes({
      ...
      getCosmosSigner: async (chainId) => {
        return yourCosmosWallet.getOfflineSigner(chainId);
      },
      getEvmSigner: async (chainId) => {
        return yourEvmWallet.getWalletClient(chainId);
      },
...
async function executeSwapWithGasOnReceive(
  ...
  await executeMultipleRoutes({
    ...
    getCosmosSigner: signers.getCosmosSigner,
    getEvmSigner: signers.getEvmSigner,
    onRouteStatusUpdated: (status) => {
...
class GasOnReceiveManager {
  ...
  private handleRouteStatus(status: RouteDetails) {
