...
The connectedWallet prop, which allowed passing a custom wallet provider, isn't currently supported. Use `connectedAddresses` with the signer functions instead (see [Connected Wallet](./connected-wallet)).
