...
Prioritizes assets by recommended symbol when their pinning, search-match, and balance priorities are equal. Symbols are ordered as listed. To pin an asset above search matches and balances, use [`assetAnnotations`](#assetannotations) with `selector.pinToTop` instead.
...
assetAnnotations?: Record<string, {
  ...
  swapPage?: {
    label: string;                         // badge on the swap page asset rows
  };
  selector?: {
