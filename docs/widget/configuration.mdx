...
```ts
routeConfig?: {
  experimentalFeatures?: string[]; // e.g. ['stargate', 'eureka']
  allowMultiTx?: boolean;
  allowUnsafe?: boolean;
  bridges?: ('IBC' | 'AXELAR' | 'CCTP' | 'CCTP_V2' | 'HYPERLANE' | 'OPINIT' | 'GO_FAST' | 'STARGATE' | 'LAYER_ZERO' | 'EUREKA')[];
  swapVenues?: {
    name: string;
    chainId: string;
  }[];
...
Controls the corner roundness of cards and buttons in the widget. This is not a top-level prop — set it under [`theme`](#theme) via `theme.borderRadius`.
...
List of asset symbols to pin to the top of the token selector.
...
Render a per-asset annotation (badge / warning icon / highlight / pin-to-top), keyed by recommended symbol.

```ts
assetAnnotations?: Record<string, {
  variant?: "info" | "warning" | "error"; // color; defaults to "info"
  swapPage?: {
    label: string;                         // badge + warning icon on the swap page asset rows
  };
  selector?: {
    description?: string;                   // detail line under the asset name in the token selector
    pinToTop?: boolean;                     // pin the asset to the top of the selector, above balances
  };
}>;
```

- `variant`: color of the badge / icon / highlight. `info` uses the brand color, `warning` and `error` use the theme's warning/error colors.
- `swapPage.label`: renders a badge and a warning icon next to the asset name on the swap page (source and destination rows).
- `selector.description`: detail line with a warning icon under the asset name in the token selector.
