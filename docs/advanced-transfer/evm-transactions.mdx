...
### 1\. Initialize the `SkipRouter` client with the EVM `WalletClient` object
...
You need to set up the `getEVMSigner` function in the `SkipRouter` constructor to initialize this signer object for the a given EVM chain.
...
### 2\. Request Route using `SkipRouter` and get required chain
...
### 4\. Execute the Route using `SkipRouter`

Finally, you can use `SkipRouter.executeRoute` to prompt the user to sign the approval(s) and transaction, and submit the transaction on chain. 
