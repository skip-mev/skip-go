...
**Open-Source Reference Solver Implementation:** To make it as easy as possible to get started, we've open-sourced a reference implementation of a solver that handles everything from monitoring events, filling orders, settling, and rebalancing. All you need to do is set up the config with the chains you want to solve for, provide node endpoints to use, and customize your capital and rebalancing preferences. Check out the repo [here](https://github.com/skip-mev/skip-go-fast-solver).
...
If you have any questions about setting up a solver, please don't hesistate to reach out to us! 
...
The source and destination gas fees are determined dynamically based on the source and destination chains and their current gas costs, optimized to be as low as possible while still covering settlement and rebalance costs for the solvers.