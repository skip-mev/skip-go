...
We strongly reccomend using the `@skip-go/client` [TypeScript package](https://www.npmjs.com/package/@skip-go/client), which is actively maintained.
...
Set your Smart Swap settings in your `skipClient` function call our REST API request body.
...
Set the `evmSwaps` flag to true in the `smartSwapOptions` object. If using the deprecated `@skip-router` library, you must be on v5.1.0+ (we strongly reccomend migrating to `@skip-go/client` as soon as possible).
