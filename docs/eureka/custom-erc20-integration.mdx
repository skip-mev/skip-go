...
In the initial release of [`solidity-ibc-eureka`](https://github.com/cosmos/solidity-ibc-eureka), receiving a non-native token (e.g., ATOM from Cosmos Hub) results in the deployment of a default [`IBCERC20`](https://github.com/cosmos/solidity-ibc-eureka/blob/main/contracts/utils/IBCERC20.sol) contract to represent that token on Ethereum.

However, many teams bridging through Cosmos Hub want ownership and control over the ERC20 contracts representing their tokens on Ethereum. Because `IBCERC20` is managed by the `ICS20Transfer` contract and not externally customizable, direct ownership is not possible.

To support external teams taking control of their bridged tokens, we now allow them to deploy their own ERC20 contracts—provided they implement a simple interface that permits the `ICS20Transfer` contract to mint and burn tokens.
...
<CardGroup cols={2}>
  ...
  <Card title="CEX Listing" icon="building-bank">
...
To call this function, an address must hold the `ERC20_CUSTOMIZER_ROLE`, which is managed exclusively by the protocol's security council.
...
<Steps>
  ...
  <Step title="Contact the Team">
    Reach out to [Jeremy](https://t.me/NotJeremyLiu) or [Susannah](https://t.me/bigsuse) to coordinate the registration of your custom ERC20 contract.
  </Step>
  <Step title="Register Before First Transfer">
    Ensure your custom ERC20 is registered via `setCustomERC20` before any IBC transfers of your token occur.
  </Step>
...
- Contact us directly: [Jeremy](https://t.me/NotJeremyLiu) or [Susannah](https://t.me/bigsuse) on Telegram
