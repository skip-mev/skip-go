...
For users of the `@skip-go/client` TypeScript package, the `apiKey` can be included in the `SkipRouterOptions` object when constructing a `SkipRouter`. After you include it in construction, the `SkipRouter` will handle passing it in the `authorization` header of all requests. 
