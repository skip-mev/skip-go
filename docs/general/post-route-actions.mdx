...
Callers of the Skip Go API can specify actions to execute on the destination chain after a route transfer or swap is complete using the `post_route_handler` parameter of `/v2/fungible/msgs`as a part of the same transaction where the swap/transfer took place. 
