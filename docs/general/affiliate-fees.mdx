...
- Importance: You'll need valid addresses on this chain for the next step.
...
By correctly setting `cumulative_affiliate_fee_bps` in your `/route` request and specifying the fee distribution in the `affiliates` array in your `/msgs` request, you can effectively incorporate fees into your swap transactions.

- **Ensure:**
  - The total `basis_points_fee` matches the `cumulative_affiliate_fee_bps`.
  - All addresses are valid on the swap chain.

**Failure to meet these requirements will result in a 400 error.**
