...
For example, if one of your end users initiates a swap that begins with USDC on Noble and concludes with wstETH on Neutron, you can use the lifecycle tracking to report when the USDC moves from Noble to the Osmosis, where USDC is swapped to wstETH, then IBC'd to Neutron. It tracks every step of the journey.

<Frame>![](/images/Widget-tracking-tx2025.png)</Frame> 
