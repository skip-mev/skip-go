...
## Skip Explorer v2 Integration for Custom Frontends

If you're building a custom frontend and want to provide users with rich transaction visualization in Skip Explorer v2, you can generate explorer links that include comprehensive route data.

### Basic Explorer Links

For simple transaction tracking, you can direct users to the explorer with basic URL parameters:
...
The advanced integration provides:

1. **Complete Transaction Context**: Users see the full route, not just individual transactions
2. **Multi-Hop Visualization**: Clear view of complex transfers across chains and bridges  
3. **Address Mapping**: Explorer knows which addresses belong to the user
4. **Real-time Status**: Current transaction state integrated with visual progress
5. **Shareable Links**: Users can bookmark or share complete transaction context

<Info>
**Best Practice**: Use the advanced base64 approach when you have access to complete route information from your Skip Go integration. This provides the richest user experience in the explorer.
</Info>
