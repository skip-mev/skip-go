...
This document introduces Skip Go API’s Smart Relay functionality — the fastest & most reliable way to get your users where they’re going over any bridge supported by Skip Go. Smart Relay is an alternative to public bridge relayers. Smart Relayers are designed to enable users to access more swap and transfer routes in fewer transactions more quickly & reliably.
